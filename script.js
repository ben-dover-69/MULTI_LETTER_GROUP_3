document.addEventListener("DOMContentLoaded", () => {
  ...
  sendBtn.onclick = () => {
    ...
    const letterBgColor = letterColors[letterType.value] || "#fde2e2";
    const letterFont = activeLetter.style.fontFamily || "'Playfair Display', serif";

    // Generate view link safely
    const viewLink = `https://ben-dover-69.github.io/MULTI_LETTER_GROUP_3/view.html?recipient=${encodeURIComponent(email)}&letter_type=${encodeURIComponent(letterType.value)}&letter_content=${encodeURIComponent(letterContent)}&letter_title=${encodeURIComponent(letterTitle)}&sender_name=${encodeURIComponent(senderNameValue)}&letter_type_bg=${encodeURIComponent(letterBgColor)}&letter_title_font=${encodeURIComponent(letterFont)}`;

    // Smooth wax stamp animation
    ...
    const templateParams = {
      ...
      letter_type_bg: letterBgColor,
      letter_title_font: letterFont,
