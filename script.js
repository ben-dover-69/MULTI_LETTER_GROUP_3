document.addEventListener("DOMContentLoaded", () => {
  ...
  // Send letter with enhanced stamp animation **and fixed content**
  sendBtn.onclick = () => {
    ...
    // --- FIX: Collect actual typed content for sending ---
    const letterContent = `
      <p><strong>Dear ${activeLetter.querySelector("#receiverName")?.textContent || "Someone"},</strong></p>
      <h1>${activeLetter.querySelector("#title")?.textContent || ""}</h1>
      <div>${activeLetter.querySelector("#body")?.innerHTML || ""}</div>
      <p>Sincerely,<br>${activeLetter.querySelector("#senderName")?.textContent || "Your Name"}</p>
    `;
